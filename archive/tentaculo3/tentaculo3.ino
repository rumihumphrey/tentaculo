...
ChaseAnimation animations[10];  // Allow up to 10 simultaneous chase animations
...
void handleUdpMessage(String message) {
  ...
  animations[slot].position = animations[slot].reverse ? NUMPIXELS - 1 : 0;
...
void updateChase(int index) {
  if (millis() - animations[index].lastUpdate >= animations[index].delay) {
    ...
    for (int j = 0; j < animations[index].width; j++) {
      int clearPos = animations[index].reverse ? (animations[index].position + j + 1) % NUMPIXELS : (animations[index].position - j - 1 + NUMPIXELS) % NUMPIXELS;
